...
test.describe('Edge Cases', () => {
  ...
  test('TC-015: Special characters are preserved in todo text', async ({ page }) => {
  ...
  test('TC-017: Very long todo text is handled gracefully', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
  ...
  test('TC-018: "Mark all as complete" toggles all items at once', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
  ...
  test('TC-019: "Clear completed" removes all completed items', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    for (const item of TODO_ITEMS) {
      await input.fill(item);
      await input.press('Enter');
  ...
  test('TC-020: Double-clicking a todo enables inline editing', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    await input.fill('Buy groceries');
    await input.press('Enter');
  ...
  test('TC-021: Editing a todo and pressing Enter saves changes', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    await input.fill('Buy groceries');
    await input.press('Enter');
  ...
  test('TC-022: Editing a todo and pressing Escape cancels changes', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    await input.fill('Buy groceries');
    await input.press('Enter');
  ...
  test('TC-023: Editing a todo to empty string removes it', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    await input.fill('Buy groceries');
    await input.press('Enter');
  ...
  test('TC-025: Leading and trailing whitespace is trimmed from todo text', async ({ page }) => {
    const input = page.getByPlaceholder('What needs to be done?');
    await input.fill('   Buy groceries   ');
    await input.press('Enter');
