...
test.describe('Positive Flows', () => {
  ...
  test.describe('AC 1: Create a todo list', () => {
    test('TC-001: Todo list is created when the first item is added', async ({ page }) => {
  ...
  test.describe('AC 2: Add items (4)', () => {
    test('TC-002: Four todo items can be added sequentially', async ({ page }) => {
      const input = page.getByPlaceholder('What needs to be done?');
    ...
    test('TC-003: Input field is cleared after adding a todo', async ({ page }) => {
      const input = page.getByPlaceholder('What needs to be done?');
      await input.fill('Buy groceries');
      await input.press('Enter');
  ...
  test.describe('AC 3: Finish item — Expect to be finished', () => {
    ...
    test('TC-004: Clicking the toggle checkbox marks a todo as completed', async ({ page }) => {
    ...
    test('TC-005: Completed items appear in the "Completed" filter', async ({ page }) => {
      const firstItem = page.getByTestId('todo-item').first();
      await firstItem.getByRole('checkbox').check();
    ...
    test('TC-006: Completed item can be unchecked to reactivate', async ({ page }) => {
      const firstItem = page.getByTestId('todo-item').first();
      await firstItem.getByRole('checkbox').check();
      await expect(page.getByText('3 items left')).toBeVisible();
  ...
  test.describe('AC 4: Remove item from the list — Expect to be removed', () => {
    ...
    test('TC-007: Hovering over a todo reveals the destroy button', async ({ page }) => {
    ...
    test('TC-008: Clicking the destroy button removes the item from the list', async ({ page }) => {
      const thirdItem = page.getByTestId('todo-item').nth(2);
      await thirdItem.hover();
    ...
    test('TC-009: Removing the last item hides the list and footer', async ({ page }) => {
      for (let i = 0; i < 4; i++) {
